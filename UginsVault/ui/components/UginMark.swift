...
//
//  The Ugin's Vault sigil: a gold "U" cradle holding a hedron (rhombus)
//  diamond. Drawn via `Canvas` so it scales to any size without raster loss.
//
//  This mark is a placeholder — final logo direction is still TBD. Don't
//  invest heavy time polishing it until the user signs off on the final.
//
...
public struct UginMark: View {
    ...
    public init(size: CGFloat = 80, showsGlow: Bool = true) {
        self.size = size
    ...
    public var body: some View {
        ZStack {
            ...
            Canvas { ctx, _ in
                ...
                ctx.stroke(facets, with: .color(goldHi.opacity(0.55)), lineWidth: s * 0.007)
...
/// Rounded-square wrapper for splash / login / home-screen mocks.
public struct UginAppIcon: View {
